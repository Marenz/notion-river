...
- `notion-rofi-launch` — packaged app launcher (`config-examples/notion-rofi-launch`, installed to `/usr/bin`). Runs `rofi -show drun -m <output>` on the focused output. App launching only; does not reimplement drun. Set as `commands.launcher = ["notion-rofi-launch"]` (bound to `Super+o`).
...
- **rofi window switcher does not jump**: rofi-wayland's `combi` mode swallows the script-modi selection callback, so a window picked from a combi view never gets focused. Keep app launching (`drun`) and window switching (the `notion-rofi-window-mode` script-modi) as **separate rofi invocations on separate keybinds**, not merged via combi or a custom all-in-one modi. The built-in `window` modi needs `zwlr_foreign_toplevel_manager_v1`, which River does not serve (it serves `ext-foreign-toplevel-list-v1`, list-only) — so generic window switchers cannot work; routing through `notion-ctl` is the supported path. Do **not** reimplement `drun` in a script (it mis-launches DBusActivatable/Terminal entries); use rofi's native `drun`. Upstream River activation support is tracked in river#1281 (xdg-activation → WM event, accepted but unscheduled) — note that even when it lands it covers *activation requests*, not window *enumeration*, so it will not enable `rofi -show window`.
