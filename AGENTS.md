...
- **Drop zones** (`pointer_ops.rs`): a drop lands in one of six zones. Each edge owns a narrow band — 5% of the frame's extent, clamped to 10..60px (`SPLIT_BAND_FRACTION`/`MIN`/`MAX`) — that triggers a split; in the corners the *proportionally* nearest edge wins. The whole remaining center tabs (upper half) or swaps (lower half). The bands are deliberately thin: splitting on drop is rare, and the old 25% bands made accidental splits the common case.
- **Drop preview** (`DropZone::preview_rect`): the highlight is NOT the hit region. Split zones highlight the half the window will actually occupy after the split, so you see the outcome rather than the sliver you had to aim at. Tab and swap are inset by band + `PREVIEW_GUTTER` (5px) and stacked with the same gutter between them, so the whole cell never lights up as one blob. Color per zone: tab blue, split green, swap peach. Each box carries a centered action label (`DropZone::label`, e.g. "Add as tab" / "Swap windows" / "Split left") drawn by `decorations::draw_centered_label` — a sibling of `draw_text` that composites source-over in *premultiplied* space so the overlay stays translucent (`draw_text` force-opaques its output and would have blanked the tint). The label sits on a rounded pill (`fill_label_backdrop`) filled with the zone's hue at alpha `0xd8` instead of the fill's `0x40`, so the text reads regardless of the window content showing through; the pill is measured from the layout's *unconstrained* ink size, taken before `set_width` (with a width set, the logical extents span the whole box). `decorations::DragPreview::show` is otherwise a pure renderer — it takes the already-computed area and only reads the zone for color and label.
- **Swap drop**: trades the dragged window with the target frame's *active* window only; both frames keep their tab order, active index, and MRU focus history (`Frame::replace_window` swaps the id in-place in `focus_history`). A swap onto an empty frame, or onto the frame the window came from, has nothing to trade with and degrades to a plain tab move.
- **Split placement**: `SplitNode::split_frame_at(.., new_first)` chooses which side the *new* frame lands on. For a Top/Left drop the new frame becomes the first child, so the existing frame keeps its id and windows and simply moves down/right — nothing that refers to a frame by id (app bindings, saved state) has to be rewritten. Before this, Top and Left silently placed the window on the wrong side.
...
- **Reassign must be retried from `wl_output::Description`**: `maybe_reassign_outputs` is gated on EVERY connected output having geometry AND a stable monitor key, so it is a no-op until the last monitor's metadata lands. The identity comes from the `wl_output` description (EDID), which routinely arrives AFTER `Dimensions` and `Name` — and for an external monitor `Name` (`DP-8`) is not a stable key. So every earlier call bails and, unless `Description` also calls `maybe_reassign_outputs`, nothing ever retries: the monitor stays with no workspace and rejects windows moved to it (fixed 2026-08-04; symptom was one of three monitors permanently workspace-less after a dock replug).
...
- Contour terminal grid/size bugs (≤ 0.7.0-master-38e543a7) are fixed in `~/repos/contour` on the `fix-implicit-size-binding` branch; rebuild with `cmake --build out/gcc-release --target contour -j` and atomic-replace `~/.local/bin/contour` (the running binary is busy — `cp` to `contour.new` then `mv -f`). Only newly-spawned terminals pick up the new binary. **Do not work around either bug in notion-river** — notion-river proposes the correct tile size (verified: contour commits exactly `last_proposed_*`); the faults are contour-side grid handling:
